...
【最終回覆原則】
1. 只回答洗衣相關問題
2. 遇到無關問題，回覆：UNRELATED
3. 語氣簡潔親切、專業貼心
4. 適度使用 emoji：💙🧺✨
5. 汙漬相關必須強調「不保證完全去除」
6. 完全不要提供電話號碼
7. **絕對不要詢問客戶地址**
8. **絕對不要說「我無法」「我不能」「建議您聯絡」等推卸責任的話**
9. **遇到無法處理的問題，說「營業時間會有專人協助您 💙」**
`;
...
async function handleImageMessage(imageBuffer) {
  try {
    ...
    console.error('[Claude Vision] 圖片分析錯誤:', error);
